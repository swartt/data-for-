...
funnel AS (
    SELECT
        ...
        COUNT(DISTINCT CASE WHEN optin_flag = 1 THEN phone END) AS optin_cnt
    FROM cohort_window
    GROUP BY period
),

apply_agg AS (
    SELECT period, COUNT(DISTINCT phone) AS apply_cnt
    FROM apply_detail
    GROUP BY period
),

pass_agg AS (
    SELECT period, COUNT(DISTINCT phone) AS pass_cnt
    FROM pass_detail
    GROUP BY period
),

quota_agg AS (
    SELECT period, COUNT(DISTINCT phone) AS effective_quota_cnt
